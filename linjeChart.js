const SCB_LINE_DIREKT_URL =
  "https://statistikdatabasen.scb.se/api/v2/tables/TAB4312/data" +
  "?lang=sv" +
  ...
  return lineYears.map((year, yearPos) => {
    return Object.values(regionIndex).reduce((sum, regionPos) => {
      return Object.values(explIndex).reduce((s, explPos) => {
        const idx =
          ((regionPos - offset) * numExpl + explPos) * numYears + yearPos;
  ...
  return {
    ...
    datasets: [
      ...
      },
      {
        ...
        data: indirektEndast,
        backgroundColor: "rgba(255, 161, 107, 0.6)",
        borderColor: "#ffa16b",
        borderWidth: 2,
      },
    ],
